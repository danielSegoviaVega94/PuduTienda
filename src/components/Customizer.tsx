...
export default function Customizer({ box, onBack }: CustomizerProps) {
  const { products, availableSwaps, extras: availableExtras, createOrder, settings } = useAppContext();

  // Initialize cart with base items (filter out deleted products)
  ...
  const priceDifference = useMemo(() => {
    ...
    box.baseItems.forEach(bi => {
      const product = products[bi.productId];
      if (!product) return;
      const stillInCart = items.find(item => item.product.id === bi.productId);
      if (!stillInCart) {
        diff -= bi.defaultQty * product.basePrice;
      }
    });
  ...
  const handleCheckout = () => {
    ...
    const order = createOrder({
      ...
      boxTemplateName: box.name,
      boxBasePrice: box.basePrice,
      items: [
    ...
    let text = `¡Hola amigos del Pudú! 🦌 Pedido *${order.id}* — *${box.name}*.\n\n`;
    ...
    window.open(`https://wa.me/${settings.whatsappPhone}?text=${encodeURIComponent(text)}`, '_blank');
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-32">
                  ...
                  <div className="mt-auto">
                    {cartExtra ? (
                      <div className="flex items-center justify-between bg-pudu-yellow rounded-lg overflow-hidden border border-pudu-yellow-dark">
                        <button onClick={() => {
                          const idx = extras.findIndex(e => e.product.id === extra.id);
                          if (idx >= 0) handleUpdateQty(idx, -extra.step, true);
                        }} className="p-1.5 hover:bg-pudu-yellow-dark text-pudu-earth">
                          <Minus className="w-4 h-4" />
                        </button>
      ...
      <BottomSheet
        ...
        currentItem={swapTargetIndex !== null ? items[swapTargetIndex].product : Object.values(products)[0]}
